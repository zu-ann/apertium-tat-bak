...
\item A \emph{morphological analyser} which segments the text in
  ...
  units or MWLUs) and delivers, for each of them, one or more \emph{lexical
    forms} (LF) consisting of \emph{lemma}, \emph{lexical category} and
  morphological information. It reads a finite-state
  transducer (FST) compiled from a source-language (SL) morphological
  dictionary in XML.
\item A \emph{morphological disambiguator} (based on constraint grammar) which chooses, using linguistic rules
...
\item A \emph{morphological generator} which delivers a TL SF
 for each TL LF, by suitably inflecting it. It reads
  an FST compiled from a TL morphological dictionary in XML.
...
\hline
{\bf Input}& Һава бүген бик әйбәт, җылы гына \\ 
\hline
...
{\bf Lexical transfer}& \^{}Һава\tag{<n>}\tag{<attr>}/Һауа\tag{<n>}\tag{<attr>\$} \^{}бүген\tag{<adv>}/бөгөн\tag{<adv>\$} \^{}бик\tag{<adv>}/бик\tag{<adv>\$} \\
 ~                    & \^{}әйбәт\tag{<adj>}/әйбәт\tag{<adj>\$}\^{},\tag{<cm>}/,\tag{<cm>\$} \^{}җылы\tag{<n>}\tag{<attr>}/йылы\tag{<n>}\tag{<attr>\$} \\ 
...
\hline
\end{tabular}
 \caption{Translation process from Tatar to Bashkir of the phrase `Һава бүген бик әйбәт, җылы гына' {\em Today the weather is very nice, it is just warm'.}}
\end{table*}
...
\begin{table}
  ...
  \begin{tabular}{c|r|r}
   ...
   \hline
   Tatar Wikipedia         & 37,123    & 64.86\% \\
   Tatar New Test.         & 163,603   & 69.65\% \\
   \hline
