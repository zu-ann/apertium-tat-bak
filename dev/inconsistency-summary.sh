...
POS="adj adv cm cnjcoo det guio ij n np num post postadv prn v"
...
for i in $POS; do
	if [ "$i" = "det" ]; then
		...
		TOTAL=`cat $INC | grep "<$i>" | grep -v -e '<part>' -e '<qst>' -e '<v' -e '<adj>' | grep -v REGEX | wc -l`; 
		AT=`cat $INC | grep "<$i>" | grep '@' | grep -v -e '<part>' -e '<qst>' -e '<v' -e '<adj>'  | grep -v REGEX | wc -l`;
		HASH=`cat $INC | grep "<$i>" | grep '>  *#' | grep -v -e '<part>' -e '<qst>' -e '<v' -e '<adj>' | grep -v REGEX |  wc -l`;
	elif [ "$i" = "cnjcoo" ]; then
		TOTAL=`cat $INC | grep "<$i>" | grep -v -e '<v>' -e '<n>' | grep -v REGEX | wc -l`; 
		AT=`cat $INC | grep "<$i>" | grep '@' | grep -v -e '<v>' -e '<n>'  | grep -v REGEX | wc -l`;
		HASH=`cat $INC | grep "<$i>" | grep '>  *#' | grep -v -e '<v>' -e '<n>' | grep -v REGEX |  wc -l`;
	elif [ "$i" = "np" ]; then
		TOTAL=`cat $INC | grep "<$i>" | grep -v -e '<part>' -e '<qst>' | grep -v REGEX | wc -l`; 
		AT=`cat $INC | grep "<$i>" | grep '@' | grep -v -e '<part>' -e '<qst>'  | grep -v REGEX | wc -l`;
		HASH=`cat $INC | grep "<$i>" | grep '>  *#' | grep -v -e '<part>' -e '<qst>' | grep -v REGEX |  wc -l`;
	elif [ "$i" = "n" ]; then
		TOTAL=`cat $INC | grep "<$i>" | grep -v -e '<part>' -e '<qst>' | grep -v REGEX | wc -l`; 
		AT=`cat $INC | grep "<$i>" | grep '@' | grep -v -e '<part>' -e '<qst>'  | grep -v REGEX | wc -l`;
		HASH=`cat $INC | grep "<$i>" | grep '>  *#' | grep -v -e '<part>' -e '<qst>' | grep -v REGEX |  wc -l`;
